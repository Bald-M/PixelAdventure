...
func _ready():
	...
	diamond_total_changed(get_tree().get_nodes_in_group("diamond").size())
...
func diamond_collected():
	collectedDiamonds += 1
	emit_signal("diamond_total_changed", totalDiamonds, collectedDiamonds)

func diamond_total_changed(newTotal):
